...
clientRouter.post('/register', zValidator('json', RegisterSchema), async (c) => {
...
clientRouter.post('/heartbeat', zValidator('json', HeartbeatSchema), async (c) => {
  const data = c.req.valid('json')
  ...
  const openEvent = db.select({ id: networkEvents.id, offlineAt: networkEvents.offlineAt })
    .from(networkEvents)
    .where(and(eq(networkEvents.pcId, pc.id), eq(networkEvents.onlineAt, null as any)))
    .get()
...
clientRouter.get('/commands', async (c) => {
...
clientRouter.post('/commands/:id/result', zValidator('json', CommandResultSchema), async (c) => {
...
clientRouter.post('/offline', zValidator('json', OfflineSchema), async (c) => {
  const data = c.req.valid('json')
  ...
  const existing = db.select({ id: networkEvents.id }).from(networkEvents)
    .where(and(eq(networkEvents.pcId, pc.id), eq(networkEvents.onlineAt, null as any))).get()
...
clientRouter.post('/shutdown', zValidator('json', z.object({ machine_id: z.string() })), async (c) => {
...
clientRouter.get('/version', async (c) => {
...
clientRouter.post('/version', async (c) => {
