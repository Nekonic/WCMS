...
// 실습실 생성
roomsRouter.post('/', requireAdmin, zValidator('json', RoomSchema), async (c) => {
...
// 실습실 수정
roomsRouter.put('/:id', requireAdmin, zValidator('json', RoomSchema.partial()), async (c) => {
...
roomsRouter.get('/:room/layout', async (c) => {
...
roomsRouter.post('/:room/layout', requireAdmin, zValidator('json', LayoutSaveSchema), async (c) => {
  const roomName = c.req.param('room')
