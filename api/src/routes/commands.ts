...
export function createCommandsRouter(db: DB) {
  const router = new Hono()

  // 대기 명령 목록
  router.get('/pending', requireAdmin, async (c) => {
  ...
  router.post('/results', requireAdmin, zValidator('json', ResultsQuerySchema), async (c) => {
  ...
  router.post('/bulk', requireAdmin, zValidator('json', BulkCommandSchema), async (c) => {
  ...
  // 여러 PC 명령 큐 삭제
  router.delete('/bulk', requireAdmin, zValidator('json', z.object({ pc_ids: z.array(z.number().int()) })), async (c) => {
