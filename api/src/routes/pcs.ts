...
export const pcsRouter = new Hono()
...
// 전체 목록 (관리자)
pcsRouter.get('/', requireAdmin, async (c) => {
  markOfflineIfStale()
...
// 중복 호스트명
pcsRouter.get('/duplicates', requireAdmin, async (c) => {
...
// 미검증 PC
pcsRouter.get('/unverified', requireAdmin, async (c) => {
  const pcs = db.select().from(pcInfo).where(eq(pcInfo.isVerified, false)).all()
  return c.json(pcs)
})

// PC 상세
pcsRouter.get('/:id', requireAdmin, async (c) => {
  markOfflineIfStale()
...
// PC 프로세스 기록
pcsRouter.get('/:id/history', requireAdmin, async (c) => {
...
// PC 삭제
pcsRouter.delete('/:id', requireAdmin, async (c) => {
  const id = Number(c.req.param('id'))
...
// 범용 명령
pcsRouter.post('/:id/command', requireAdmin, zValidator('json', CommandSchema), async (c) => {
  const pcId = Number(c.req.param('id'))
...
pcsRouter.post('/:id/shutdown',         ...quickCommand('shutdown'))
pcsRouter.post('/:id/restart',          ...quickCommand('restart'))   // reboot 통합
pcsRouter.post('/:id/message',          ...quickCommand('show_message', z.object({ message: z.string(), duration: z.number().optional() })))
pcsRouter.post('/:id/kill-process',     ...quickCommand('kill_process', z.object({ process_name: z.string() })))
pcsRouter.post('/:id/install',          ...quickCommand('install', z.object({ package_name: z.string() })))
pcsRouter.post('/:id/uninstall',        ...quickCommand('uninstall', z.object({ package_name: z.string() })))
pcsRouter.post('/:id/account/create',   ...quickCommand('create_user', z.object({ username: z.string(), password: z.string(), language: z.string().optional() })))
pcsRouter.post('/:id/account/delete',   ...quickCommand('delete_user', z.object({ username: z.string() })))
pcsRouter.post('/:id/account/password', ...quickCommand('change_password', z.object({ username: z.string(), new_password: z.string() })))

// 명령 큐 삭제
pcsRouter.delete('/:id/commands', requireAdmin, async (c) => {
  const pcId = Number(c.req.param('id'))
  db.delete(commands)
