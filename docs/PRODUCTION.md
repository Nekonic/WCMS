...
sudo chown $USER:$USER /opt/wcms

# 저장소 클론 및 의존성 설치 (현재 유저 권한으로 실행)
# 소유권 이전은 DB 초기화(섹션 4) 이후에 함께 진행
git clone https://github.com/Nekonic/WCMS.git /opt/wcms
cd /opt/wcms
...
```bash
# 디렉토리 생성
sudo mkdir -p /var/lib/wcms /var/log/wcms
sudo chown $USER:$USER /var/lib/wcms

# DB 초기화 — chown 전에 현재 유저로 실행 (uv가 .venv에 쓰기 필요)
cd /opt/wcms
export $(sudo cat /etc/wcms/env | grep -v '^#' | xargs)
python3 manage.py init-db <관리자ID> <비밀번호> --force

# 소유권 일괄 이전 (wcms 서비스 유저로) — init-db 완료 후 실행
sudo chown -R wcms:wcms /opt/wcms /var/lib/wcms /var/log/wcms
```
...
curl -X POST http://localhost:5050/api/client/version \
  ...
  -d '{
    "version": "0.9.8",
    "download_url": "https://github.com/Nekonic/WCMS/releases/download/client-v0.9.8/WCMS-Client.exe",
    "changelog": "v0.9.8 릴리스"
  }'
```
...
# {"version":"0.9.8","download_url":"https://...","status":"success"}
```
...
```bash
# 1. 서비스 중지
sudo systemctl stop wcms

# 2. 저장소 업데이트 (현재 유저로 실행)
cd /opt/wcms
sudo chown -R $USER:$USER /opt/wcms
git pull
...
# 5. DB 마이그레이션 필요 시 (스키마 변경)
# schema.sql 변경 내역을 CHANGELOG.md에서 확인 후 수동 적용
# (데이터 전체 삭제 주의)
# export $(sudo cat /etc/wcms/env | grep -v '^#' | xargs)
# python3 manage.py init-db --force
