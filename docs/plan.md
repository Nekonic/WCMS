...
- [-] Django<->Rust 경계 구현 (무상태 게이트웨이: Django 내부 API, 양방향) — Django /internal/(presence·telemetry·pending-commands) + 게이트웨이 DjangoClient/push 엔드포인트 + Django 발행 시 gateway push(온라인) / drop 폐기(오프라인). 양방향 완성
