...
| 명령어 | 설명 |
|--------|------|
| `python3 manage.py run` | 서버 실행 (개발 모드) |
| `python3 manage.py run --prod` | 서버 실행 (Gunicorn, 프로덕션) |
| `python3 manage.py test` | 단위 테스트 실행 |
| `python3 manage.py build` | 클라이언트 EXE 빌드 (Windows 전용) |
| `python3 manage.py init-db [id] [pw]` | DB 초기화 (기본값: admin / admin) |
| `python3 manage.py install` | 의존성 설치 |
...
`python3 manage.py build`는 Windows 환경에서만 실행 가능합니다 (pywin32, WMI 의존성 필요).
